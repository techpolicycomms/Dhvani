...
0. ✅ **Transcription is mode-routed** — FIXED 2026-04-20. Mic mode uses **local Whisper** in-browser via [`lib/localWhisper.ts`](lib/localWhisper.ts) (audio stays on-device, $0 per minute, no diarization needed because single-speaker). Meeting modes (tab-audio / electron / virtual-cable) keep using Azure `gpt-4o-transcribe-diarize` via `/api/transcribe`. Routing lives in [`hooks/useTranscription.ts`](hooks/useTranscription.ts); capture mode is read on every chunk via a ref, so switching source mid-session Just Works. Native whisper.cpp binary for Electron is a future perf upgrade (see [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) Deferred section).
0a. ✅ **Speaker identification was broken across chunks** — FIXED 2026-04-20 with a session-wide time-adjacency stitcher ([`lib/speakerStitcher.ts`](lib/speakerStitcher.ts)) + voice-embedding stitcher ([`lib/embeddingStitcher.ts`](lib/embeddingStitcher.ts) + [`lib/voiceEmbedder.ts`](lib/voiceEmbedder.ts)) and an inline merge UI on the transcript speaker list. Voice embeddings match the same speaker across long silences via cosine similarity against running centroids; `window.DHVANI_DEBUG_SPEAKERS = true` + `window.DHVANI_EMBED_THRESHOLD = 0.65` for diagnostics and live tuning.

...
0. ✅ **Transcription is intent-routed** — FIXED 2026-04-20. Home page is an [`IntentCards`](components/IntentCards.tsx) picker (what are you capturing?), not an audio-source picker:
    ...
    - **Online meeting** → tab/system audio + Azure diarize.
    Routing lives in [`hooks/useTranscription.ts`](hooks/useTranscription.ts); intent + privacy are read on every chunk via refs so switching mid-session Just Works. Local failures fall through to the cloud path. Native whisper.cpp binary for Electron is a future perf upgrade (see [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) Deferred section).
